...
def add_job_description(name: str, description: str, db_path: str = DB_PATH) -> int:
    ...
    cur.execute("INSERT OR REPLACE INTO JOB_DESCRIPTIONS (name, description, created_at) VALUES (?, ?, ?)",
...
def list_job_descriptions(db_path: str = DB_PATH) -> list:
    conn = get_connection(db_path)
    cur = conn.cursor()
...
def save_score(resume_hash: str, score: float, model_version: str = "v1", jd_id: int = None, source_filename: str = None, db_path: str = DB_PATH) -> None:
    conn = get_connection(db_path)
    cur = conn.cursor()
    now = datetime.utcnow().isoformat() + "Z"
    cur.execute(
...
def list_cached_candidates(db_path: str = DB_PATH) -> list:
    conn = get_connection(db_path)
    cur = conn.cursor()
...
# optional: force-rescore (delete then re-score)
def delete_cached(resume_hash: str, db_path: str = DB_PATH) -> None:
    conn = get_connection(db_path)
    cur = conn.cursor()
